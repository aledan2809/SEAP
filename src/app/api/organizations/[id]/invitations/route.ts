...
export async function POST(
  ...
  try {
    ...
    // Create invitation (expires in 7 days) with cryptographically secure token
    ...
    const safeOrgName = escapeHtml(membership.organization.name);
    await sendEmail({
      to: email,
      ...
      html: `
        <div style="font-family:sans-serif;max-width:500px;margin:0 auto;">
          ...
          <p><strong>${safeInviterName}</strong> te-a invitat să te alături organizației <strong>${safeOrgName}</strong> pe SEAP Assistant.</p>
